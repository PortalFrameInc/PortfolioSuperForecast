...
# Taille de l'échantillon pour CVaR/MDD au niveau du titre (affichage uniquement).
SAMPLE_SIZE_FOR_DISPLAY = 100


class Security:
    """Classe de base pour tous les types de titres financiers."""
    
    def __init__(self, name, identifier, mu, sigma, rf, conf_level, seed=None):
        self.name = name
        ...
        self.seed = seed
        self.prices = None
        self.generate_returns(self.mu, self.sigma)
    ...
    def generate_returns(self, mu, sigma):
        """
        Génère un échantillon de rendements aléatoires (distribution normale) pour CVaR/MDD au niveau du titre.

        Usage : affichage uniquement. Ces rendements ne sont pas utilisés par la simulation Monte Carlo du portefeuille.
        """
        rng = np.random.default_rng(self.seed)
        self.returns = rng.normal(mu, sigma, SAMPLE_SIZE_FOR_DISPLAY)
...
class Equity(Security):
    ...
    def __init__(self, name, identifier, mu, sigma, rf, conf_level, seed=None):
        Security.__init__(self, name, identifier, mu, sigma, rf, conf_level, seed)
        self.type = 'equity'
...
class LeveragedEquity(Equity):
    ...
    Modélise un ETF à levier (ex. 2x, 3x) avec rééquilibrage quotidien.

    Données et rendements:
    - get_price_data() : hérité de Security ; récupère les données depuis l'API avec self.identifier
      (ticker du LETF, ex. QLD pour 2x QQQ, UPRO pour 3x S&P 500), comme pour tout autre titre.
    - generate_returns() : rendements = base_equity.returns × levier (pour CVaR/MDD au niveau du titre).
    - Dans Portfolio.run_single_trajectory(), rendement de chaque période = levier × rendement sous-jacent
      (rééquilibrage à chaque période, cohérent avec les ETF à levier réels).

    ⚠️ IMPORTANT: Nécessite frequency='daily' pour capturer correctement la composition quotidienne.

    Critiques de l'implémentation naïve (mu = L * mu_base, sigma = L *sigma_base):
    - Surévaluation du rendement attendu: en réalité, la composition (compound) crée un "volatility drag":
      le rendement géométrique moyen est inférieur à L * mu_base à cause de la variance.
    - Formule usuelle (rendements i.i.d.): rendement géométrique attendu ≈ L * μ - (L² * σ²)/2.
    - Les ETF à levier réels sous-performent souvent sur horizons longs (marchés mean-reverting);
      en marchés tendanciels ils peuvent au contraire dépasser la cible.

    Améliorations apportées:
    - Ajustement du volatility drag sur mu: on utilise le rendement géométrique attendu
      mu = L * mu_base - (L² * sigma_base²)/2 pour l'affichage et la cohérence (Sharpe, etc.).
    - sigma reste L * sigma_base (volatilité des rendements de période).
    - La simulation reste "rendement période = L * rendement sous-jacent", ce qui est correct
      pour un rééquilibrage à chaque période; le drag apparaît naturellement dans les CAGRs simulés.

    Limites connues (non modélisées):
    - Autocorrélation des rendements: en marchés mean-reverting le drag est plus fort;
      en marchés tendanciels les LETF peuvent dépasser la cible.
    - Frais de gestion et coûts de réplication
    """
    ...
    def __init__(self, name, identifier, base_equity, leverage_factor, rf, conf_level, seed=None):
        ...
        # Volatilité des rendements de période: L * sigma sous-jacent
        leveraged_sigma = leverage_factor * base_equity.sigma
        
        # Rendement géométrique attendu (volatility drag): L * μ − (L² * σ²)/2
        # Évite de surévaluer le rendement long terme des ETF à levier
        volatility_drag = (leverage_factor ** 2) * (base_equity.sigma ** 2) / 2
        leveraged_mu = leverage_factor * base_equity.mu - volatility_drag
        
        # Initialiser la classe parente
        Equity.__init__(self, name, identifier, leveraged_mu, leveraged_sigma, rf, conf_level, seed)
        self.type = 'leveraged equity'
    
    def generate_returns(self, mu, sigma):
        """
        Génère les rendements avec levier à partir du sous-jacent (pour CVaR/MDD au niveau du titre).

        Rendements du titre à levier = rendements du sous-jacent * facteur de levier.
        Utilise l'échantillon déjà généré par le base_equity pour calculer CVaR et drawdown
        affichés au niveau du LeveragedEquity.
        """
        self.returns = np.asarray(self.base_equity.returns, dtype=float).copy() * self.leverage_factor
...
class Fixedincome(Security):
    ...
    def __init__(self, name, identifier, mu, sigma, rf, conf_level, yld, duration=5.0, seed=None):
        """
        ...
        Security.__init__(self, name, identifier, mu, bond_sigma, rf, conf_level, seed)
    ...
    def generate_returns(self, mu, sigma):
        ...
        size = SAMPLE_SIZE_FOR_DISPLAY
        # Composante fixe: le yield
        fixed_component = np.full(size, self.yld)
        
        # Composante variable: variation du prix (beaucoup plus faible que le yield)
        rng = np.random.default_rng(self.seed)
        price_volatility = sigma * 0.5
        price_variation = rng.normal(0, price_volatility, size)
...
class Option(Security):
    ...
    def __init__(self, name, identifier, mu, sigma, rf, conf_level, option_type, strike_price, expiration_date, seed=None):
        Security.__init__(self, name, identifier, mu, sigma, rf, conf_level, seed)
...
class Future(Security):
    ...
    def __init__(self, name, identifier, mu, sigma, rf, conf_level, future_type, direction, settlement_price, expiration_date, notional_amount, seed=None):
        Security.__init__(self, name, identifier, mu, sigma, rf, conf_level, seed)
